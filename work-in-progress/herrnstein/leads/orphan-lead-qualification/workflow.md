...
Ownership: leave Primary checked (default). Leave Second / Sales / F&I / BDC unchecked.

Orphan: check this box.

What: change from "Generated a Lead" to "Have Bought a Vehicle". (This reveals a Dates section and the Vehicle Filters.)

Dates: set the start and end date to the chosen purchase-date window.

Make: select Hyundai.

Model: select the chosen model (e.g. Tucson).

Click Search Now. Results load at the bottom (e.g. "Total Results: 414").

Click the "Last Action Date" sort link. The list re-sorts to show customers contacted longest ago first (best orphan candidates at the top).
